...
<a href="/tambah/kategori/pembayaran" class="btn btn-primary"> <i class="fa-solid fa-circle-plus pr-2"></i>Tambah Kategori
    Pembayaran</a>
@if (session('error'))
    <script>
                ...
                <table class="table datatables" id="dataTable-1">
                    ...
                    <tbody>
                        ...
                        @foreach ($kapem as $kp)
                            <tr>
                                <td>{{ $no++ }}</td>
                                <td>{{ $kp->kategori_pembayaran }}</td>
                                <td>
                                    @if ($kp->status == 'Aktif')
                                        <span class="badge badge-lg bg-success text-white"
                                            style="width: 50%; height: 100%;">{{ $kp->status }}</span>
                                    @else
                                ...
                                <td><button class="btn btn-sm dropdown-toggle more-horizontal" type="button"
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                        <a class="dropdown-item"
                                            href="/ubah/kategori/pembayaran/{{ $kp->id_kapem }}">Ubah</a>

                                        <button onclick="ubahstatusConfirmation()" class="dropdown-item"
                                            href="/ubah/status/kategori/pembayaran/{{ $kp->id_kapem }}">
                                            @if ($kp->status == 'Aktif')
                                                Non-Aktifkan
                                            @else
                                                Aktifkan
                                        ...
                                        <input type="hidden" name="idPes" id="idPes" value="{{ $kp->id_kapem }}">
                                    </div>
                                </td>
                            </tr>
                            <script>
                                function ubahstatusConfirmation() {
                                    ...
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            window.location.href = '/ubah/status/kategori/pembayaran/' + kode;
                                        }
                                    });
                                }
                            </script>
